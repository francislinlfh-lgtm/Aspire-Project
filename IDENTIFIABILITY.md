...
within-domain resolution is deterministic (a unique best match per domain), P-MIX(α)
and P-ANCHOR(p = 1−α) induce identical choice distributions: both are two-point
mixtures over {egocentric best, common-ground best} with weights (α, 1−α).
...
**Proposition 2 (nesting).** P-EGO = P-MIX(α=1) and P-CG = P-MIX(α=0). The pure
models are boundary points, not alternatives; fitting α tests them automatically.
...
|---|---|---|---|
| P-EGO vs P-MIX(α<1) | control trials (domains coincide) | critical trials | **yes** — and humans' intermediate error rates already reject P-EGO (Keysar et al., 2000) |
| P-CG vs P-MIX(α>0) | control trials | critical trials (P-CG predicts zero interference) | **yes** — fixation interference and nonzero errors already reject P-CG (Keysar et al., 2000; Hanna et al., 2003) |
...
Keysar, Lin & Barr, 2003 (verify details)). A *fixed-α* mixture predicts
...
A mixture with time-varying α(t) can mimic any serial account; flexible models mimic
