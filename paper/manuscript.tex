...
\title{From Error Curves to an Estimated Weight: A Model-Based Reanalysis of
Perspective Taking Across Adulthood}
\author{Francis Lin\\{\small VIS@betterworld lab Experimental Educational
Institution}\\{\small \texttt{francis.lin.lfh@gmail.com}}}
\date{}
...
This project was conducted by the author with substantial assistance from Claude
(Anthropic). The author developed the research question, selected the
theoretical model, made the substantive modeling and analysis decisions,
directed and froze the robustness protocol, verified the reconstructed dataset
against its published anchors, read and verified the core sources, interpreted
the results, and takes full responsibility for all claims and conclusions.
Claude served as a research and engineering assistant: it implemented and
debugged the analysis pipeline and figure code, drafted the protocol document
and the Model, Method, and Results sections for the author's verification,
performed literature searches whose key sources the author read directly, and
provided methodological critique --- including two rounds of structured
adversarial review, elicited by the author in sessions separate from the
drafting context, that produced the protocol's amendments. All numerical results were
produced by the publicly committed analysis pipeline and checked against the
preserved execution logs; the repository's commit history records this division
of labor in detail. The Introduction, Discussion, and Abstract were written by
the author, with AI feedback during revision.
% [Align final wording with the target venue's policy at submission.]
