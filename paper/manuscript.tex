...
directed and froze the robustness protocol following external review, verified
the reconstructed dataset against its published anchors, read and verified the
core sources, interpreted the results, and takes full responsibility for all
claims and conclusions. Claude served as a research and engineering assistant:
it implemented and debugged the analysis pipeline and figure code, drafted the
protocol document and the Model, Method, and Results sections for the author's
verification, and performed literature searches whose key sources the author
read directly. All numerical results were produced by the publicly committed
analysis pipeline and checked against the preserved execution logs; the
repository's commit history records this division of labor in detail. The
Introduction, Discussion, and Abstract were written by the author, with AI
feedback during revision.
% [Pending FL: identify the source of the external review (self / person /
%  another AI) and credit accordingly in Acknowledgments or this statement.
...
\paragraph{Addendum 1 (commit \texttt{a8de96b}; following external review).}
